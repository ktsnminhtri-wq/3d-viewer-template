...
chcp 65001 >nul
title GLB Model Deploy

set "EXITCODE=1"
set "SOURCE=%~f1"
set "BAT_DIR=%~dp0"
set "LOG_DIR=%~dp0logs"
set "LOG_FILE=%LOG_DIR%\deploy-model-last.log"
set "DEPLOY_MODEL_LOG=%LOG_FILE%"
set "DEPLOY_MODE_ARG="

if /I "%~2"=="--dry-run" set "DEPLOY_MODE_ARG=--dry-run"
...
if "%~1"=="" (
  set "ERROR_MESSAGE=No file received. Drag a .glb file onto DEPLOY_MODEL.bat."
  goto ERROR
)
if /I not "%~x1"==".glb" (
...
>>"%LOG_FILE%" echo Command: call node.exe --max-old-space-size=8192 "%BAT_DIR%scripts\deploy-current-model.mjs" "%SOURCE%" %DEPLOY_MODE_ARG%

call node.exe --max-old-space-size=8192 "%BAT_DIR%scripts\deploy-current-model.mjs" "%SOURCE%" %DEPLOY_MODE_ARG%
...
if not "%EXITCODE%"=="0" (
  set "ERROR_MESSAGE=DEPLOY_MODEL stopped safely. Nothing was pushed after the error."
  goto ERROR
)
